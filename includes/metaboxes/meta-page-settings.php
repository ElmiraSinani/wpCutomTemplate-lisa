...
function lis_meta_box_callback($post) {
    ...
    $footerNav = get_post_meta($post->ID, '_lis_footer_nav', true); 
    ...
    <div class="meta_item">
        <label for="lis-footer-nav">
            <input type="checkbox" name="lis-footer-nav" id="lis-footer-nav" value="yes" <?php if ( isset ( $footerNav ) ) checked( $footerNav, 'yes' ); ?> />
            <?php _e( 'Footer Nav (Men/Women Link)', 'lis_textdomain' )?>
        </label>
    </div>
    <div class="meta_item">
...
function lis_save_meta_box_data($post_id) {
    ...
    if( isset( $_POST[ 'lis-footer-nav' ] ) ) {
        update_post_meta( $post_id, '_lis_footer_nav', 'yes' );
    } else {
        update_post_meta( $post_id, '_lis_footer_nav', '' );
    }
